...
> for my test statistics, I will use # of rounds reesulting in consecutive outcomes and length of longest number of consecutive outcomes 
...
calculate_digram_range = function(flips) {
  if (length(flips) <= 1) {
    return(0)
  }
  
  counts = c(HH=0, HT=0, TH=0, TT=0)
  
  for (i in 2:length(flips)) {
    pair = paste0(flips[i-1], flips[i])
    if (pair == "HH") counts["HH"] = counts["HH"] + 1
    else if (pair == "HT") counts["HT"] = counts["HT"] + 1
    else if (pair == "TH") counts["TH"] = counts["TH"] + 1
    else if (pair == "TT") counts["TT"] = counts["TT"] + 1
  }
...
null_digram_range = seq(0, 0, length.out = N_mc)
...
for (i in 1:N_mc) {
  ...
  null_digram_range[i] = calculate_digram_range(random_flips)
...
results = data.frame(
  ...
  P_Digram_Range = seq(0, 0, length.out = 6),
...
for (i in 1:length(sequences)) {
  ...
  obs_dr = calculate_digram_range(current_flips)
  obs_c = count_consecutive(current_flips)
  obs_lc = longest_consecutive_sequence(current_flips)
  
  results$P_Digram_Range[i] = calculate_p_value(obs_dr, null_digram_range)
...
print(results)
```
> flips4 is the one that is the most likely sequence to be randomly generated because the p values for all categories on flips4 are all in general consistently significantly above average as compared to the other flip sequences. 
