...
```
> algorithm 6. The best metric is the one with the lowest squared means 
error, since mean squared error combines the bias and variance to give an overall picture of the estimator's quality. 
...
```
> the histogram is centered near the true value of p = 0.15 
...
var(p_estimates)
```
> since there is an esimated bias of .00672, there is a small positive bias
...
hist(x_bar_estimates,
     xlab = "Estimated λ using Sample Mean (X-bar)",
     col = "green",
     border = "black",
     freq = FALSE)
abline(v = true_lambda, col = "red", lwd = 2)
legend("topright", legend = "True λ = 20", col = "red", lty = 1, lwd = 2)
```
...
hist(s_squared_estimates,
     xlab = "Estimated λ using Sample Variance (S^2)",
     col = "green",
     border = "black",
     freq = FALSE)
abline(v = true_lambda, col = "red", lwd = 2)
...
```
> prefer to use Xbar because although both have a very low bias, the variance of xbar is much smaller than for S^2, and an estimator with a mlower variancve is more reliable. 
...
a_optimal
```
> since a is basically 1, the best linear combination is probably still just Xbar. A better estimator can be constructed, however the difference in improvement would be so marginally small that it is probably not worth the effort. 
...
> based on the monte carlo results, the sample mean is the best estimator for the population mean. Estimator 1 has the smalleest MSE and therefore the it provides the most accurate estimation of the true mean for this population at n = 25 
