...
> X ~ Hypergeometric(N = 150, m = 30, k = 10)

> H0: P(Paulo drawing selected) = 30/150 = .2
> Ha: P(Paulo drawing selected) > 30/150 = 0.2
...
p_value <- 1 - phyper(4, m, N_minus_m, k)    
p_value
```
> p = 0.0279 < 0.05 = alpha, reject H0. We have statistically significant evidence that Paulo's 5 winning drawings cannot be attributed to chance alone 
...
> to see if the new part genuinly changed the defect rate, we calculated the observed difference in average number of defects, which was a decrease of 1.23. Then we ran a simulation to see how likely it is to get a difference this large by random chance. To do so, we combined all defect data together to shuffle them into new random "before" and "after" groups and we found the chance to get a difference as extreme as we observed was 1.64%, and since its lower than our 5% threshold, it is highly unlikely this happened by pure chance alone and can conclude that installing the new part significantly changed the prevelence of defects. 
