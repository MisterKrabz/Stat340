...
estimate_sample_mean_variance <- function( n, M ) {
  ...
  for(i in 1:M){
    x <- c(x, run_sample_mean_trial(10))
  }
  for(i in 1:M){
    y <- c(y, mean(x) - x)
  }
...
estimate_sample_mean_variance(20, 10)
```
...
```{r}
estimate_sample_mean_variance(20, 10)
estimate_sample_mean_variance(40, 10)
estimate_sample_mean_variance(60, 10)
estimate_sample_mean_variance(80, 10)
estimate_sample_mean_variance(100, 10)
```
> Looks about correct 
...
> H0: at n = 250 the mean will be within .25 of the population mean 
>Ha: at n = 250 the mean will not be within .25 of the population mean 

```{r}
x <- c()
for(i in 1:10000){
  poisdraws <- rpois(n = 250, lambda=5.0);
    
  x <- c(x, mean(poisdraws))
}

z <- c()
for(i in 1:10000){
  num = floor(runif(n = 1, min = 1, max = 1000))
  
  poisdraws <- rpois(n = num, lambda = 5.0)
